  ...
  return (
    <div className="bg-slate-50 border-b border-slate-200 px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 text-xs">
      ...
      <div className="flex items-center flex-wrap gap-2">
        <div className="hidden sm:flex items-center gap-1.5 mr-1 flex-wrap">
          ...
          {/* New Toggle Button: Horas de Investigación / Actividades */}
          <button
            type="button"
            onClick={() => toggleOption('showActivities')}
            title="Mostrar u ocultar actividades no docentes como Horas de Investigación, Tutorías, Asesorías y Gestión"
            className={`px-2 py-1 rounded-lg border font-medium text-[11px] transition-all cursor-pointer flex items-center gap-1.5 ${
              options.showActivities
          ...
          >
            <Check className={`w-3 h-3 ${options.showActivities ? 'opacity-100 text-purple-700' : 'opacity-0'}`} />
            <FlaskConical className={`w-3 h-3 ${options.showActivities ? 'text-purple-700' : 'text-slate-400'}`} />
        ...
        <div className="relative">
          ...
          >
            ...
            <span>Filtros ({activeTogglesCount}/7)</span>
            <ChevronDown className={`w-3 h-3 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
          </button>

          {/* Mobile dropdown */}
          {isOpen && (
            <div className="sm:hidden absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-3 z-40 space-y-2 animate-in fade-in zoom-in-95 duration-150">
              ...
              <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer py-1">
                <input
                  type="checkbox"
                  checked={options.showActivities}
                  onChange={() => toggleOption('showActivities')}
                  className="rounded text-purple-600 focus:ring-purple-500"
                />
                <span className="font-semibold text-purple-950">Horas de Investigación / Actividades</span>
              </label>
